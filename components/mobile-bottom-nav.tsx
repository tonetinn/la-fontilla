...
export function MobileBottomNav() {
  ...
  return (
    <nav
      ...
      className="safe-bottom pointer-events-none fixed inset-x-0 bottom-0 z-40 lg:hidden"
    >
      <div className="pointer-events-auto mx-3 flex items-center gap-1.5 rounded-[1.15rem] border border-cream/10 bg-deep/95 p-1.5 shadow-[0_12px_40px_rgba(16,21,11,0.3)] backdrop-blur-md">
        {ITEMS.map(({ kind, label, icon: Icon, primary }) => (
          ...
          >
            <Icon aria-hidden="true" className={primary ? 'size-4' : 'size-[1.1rem]'} />
            {label}
          </button>
        ))}
      </div>
