          ...
          {FEED.map((item, i) => (
            <Reveal key={item.alt} delay={i * 55} className={item.area}>
              <a href={INSTAGRAM} target="_blank" rel="noopener noreferrer" className="group relative block h-full overflow-hidden">
                ...
                <span className="absolute bottom-4 left-4 translate-y-2 text-[0.58rem] font-semibold uppercase tracking-[0.22em] text-cream opacity-0 transition-[opacity,transform] group-hover:translate-y-0 group-hover:opacity-100">Ver no Instagram</span>
              </a>
            </Reveal>
          ))}
        </div>
